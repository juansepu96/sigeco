...
$GetAssets=$conexion->query("SELECT * from assets ORDER BY ID ASC");
...
if(isset($_POST['print'])){
    echo "<script>window.open('asientos-print.php', '_blank');</script>";
    $num1=$_SESSION['num1'];
    $num2=$_SESSION['num2'];
    $GetAssets=$conexion->prepare("SELECT * from assets WHERE ((status<>0) AND (ID BETWEEN :num1 AND :num2)) ORDER BY ID ASC");
    $GetAssets->bindParam(':num1',$num1);
...
if(isset($_POST['excel'])){
    echo "<script>window.open('asientos-excel.php', '_blank');</script>";
    $num1=$_SESSION['num1'];
    $num2=$_SESSION['num2'];
    $GetAssets=$conexion->prepare("SELECT * from assets WHERE ((status<>0) AND (ID BETWEEN :num1 AND :num2)) ORDER BY ID ASC");
    $GetAssets->bindParam(':num1',$num1);
...
<body>
    ...
    <div class="content">
        ...
        <div class="table">
            ...
            <form method="post" action="asientos.php" style="padding-left:500px;">
                ...
                <?php foreach($GetAssetRows as $Row){ ?>
                    <table style="content-align:center;text-align:center;">
                        ...
                        ?>
                            ...
                            <td style="border:0px;width:30px;"></td>
                            <?php } ?>
                            <td style="text-align:left;border:0px;width:430px;"><?php echo substr($Row['concep'],0,100);?></td>
                            <?php if($Row['type']==0){?>
